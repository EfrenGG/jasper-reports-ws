...
public class ReporteEndpoint {
	...
	public GeneraReporteResponse generaReporte(@RequestPayload GeneraReporteRequest request) {
		...
		if (request.getNombreReporte() != null && request.getNombreReporte().endsWith(".jrxml")) {
			response.setArchivo(reportesService.generaReporteJasper(request.getNombreReporte(),
					TipoArchivo.valueOf(request.getTipoArchivo().toString()), request.getDatos()));
		} else {
			...
			response.setArchivo(reportesService.generaReporteJasper(request.getNombreReporte(),
					...
					TamanoReporte.valueOf(request.getTamanoFuente().toString()), request.isHorizontal(),
					TipoArchivo.valueOf(request.getTipoArchivo().toString()), request.getDatos(),
					request.getColumnas()));
